...
curSession.authenticate(config.get('account_info', 'email'), config.get('account_info', 'pass'))
...
def getPaymentHistory(loanid, orderid, noteid, savePage = False):
    ...
    params = {'showfoliofn':'true', 'loan_id':loanid, 'order_id':orderid, 'note_id':noteid}
    ...
    for row in paymentHistory[0].find_all('tr'):
        ...
        if rowText[-2] is None or rowText[-2].strip() == "Scheduled": continue  # Don't apply this to the score.
...
if __name__ == "__main__":
    ...
    #(44755510, 76858186, 61744998)
    #(16471598, 47532684, 23837390)
    #(16512298, 48923586, 25632445)

    print(getPaymentHistory(16471598, 47532684, 23837390, True))
